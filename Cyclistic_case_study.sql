...
AS (
	SELECT *
		,((JULIANDAY(ended_at) - JULIANDAY(started_at)) * 1440) AS duration
	FROM null_cleaned
	)
	,
	-----Total numbers of member/casual riders departing from station-----
casual_depart_station
AS (
...
depart_station
AS (
	SELECT cds.start_station_name
		,cds.Casual
		,mds.Member
	FROM casual_depart_station cds
	JOIN member_depart_station mds ON cds.start_station_name = mds.start_station_name
	)
	,
...
location_dataviz_depart
AS (
	SELECT dl.start_station_name
		,ds.Casual
		,ds.Member
		,dl.dep_lat
		,dl.dep_lng
	FROM depart_station ds
	JOIN depart_latlng dl ON ds.start_station_name = dl.start_station_name
	)
	,
	------------ Total numbers of member / casual riders arriving for respective stations ----------------
casual_arrive_station
AS (
	SELECT COUNT(member_casual) AS Casual
		,end_station_name
	FROM final_table
...
arrive_station
AS (
	SELECT cas.end_station_name
		,cas.Casual
		,mas.Member
	FROM casual_arrive_station cas
	JOIN member_arrive_station mas ON cas.end_station_name = mas.end_station_name
	)
	,
...
location_dataviz_arrive
AS (
	SELECT al.end_station_name
		,ast.Casual
		,ast.Member
		,al.arr_lat
		,al.arr_lng
	FROM arrive_station ast
	JOIN arrive_latlng al ON ast.end_station_name = al.end_station_name
	)
	,
	----Summary stats-----
summary
